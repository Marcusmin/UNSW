...
class LinearModel:
    ...
    def activation(self, x):
        ...
        """
        if isinstance(x, (bool, list, np.ndarray)):
            raise ValueError('parameter cannot be a boolean, list or numpy array')
        return (1/(1+np.exp(-x)))
    ...
    def backward(self, inputs, diff):
        ...
        """
        # if  diff > 0:
        #     self.weights[0] -= self.lr * inputs[0] * diff
        #     self.weights[1] -= self.lr * inputs[1] * diff
