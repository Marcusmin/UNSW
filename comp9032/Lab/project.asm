...
RESET:
	...
	sts TCCR5A, temp1
										; Set Timer4 to Phase Correct PWM mode.
	ldi temp1, (1 << CS40)
	sts TCCR4B, temp1
...
allcol:
		ldi temp1, 0xff
		mov button_press, temp1			; initial the butten_press
		rjmp main						; scan again
		
continue:		
		out PORTF, cmask
		ldi temp1, 0x66				; slow down the scan operation.
 delay:	
		dec temp1
		brne delay
		in temp1, PINF
...
; window 1 opaque level
 led_1A2:
		...
		jmp pop_v						; jump to change w1 opaque level
...
; press nutton '3'
 num3:
		...
		jmp led_3A4						; jump to change w2 opaque level
; window 2 opaque level 
 led_3A4:
		;rcall sleep_100ms
		ldi temp1, 85					; full duty cycle is 255, 85 = 255/3, 4 level
		mul w2, temp1					; duty cycle mul w2 condition number
		mov temp1, r0
		sts OCR4AL, temp1				; change the Timer PWM duty cycle
		CLR temp1			
		STS OCR4AH, temp1				; clear high position of Timer PWM duty cycle
		jmp pop_v
...
; press nutton '4'
 num4:
		ldi temp1, 0					; justify whether in central control
		cp central, temp1
		brne return4					; if yes, wait for scanning new input
		do_lcd_command 0b10000000		; else, change control state to 'L'
		do_lcd_data 'L'
		cpi w2, 0						; if opaqe level == 0, wait for scanning new input, cannot decrease anymore
		breq return4
		dec w2							; else w2--, opaqe level decrease
		do_lcd_command 0b11000111		; move to w2 condition number postiion
		do_lcd_data_num w2				; change w2 condition number
		jmp led_3A4						; jump to change w2 opaque level		
...
; press nutton '5'
 num5:
		ldi temp1, 0					; justify whether in central control
		cp central, temp1
		brne return5					; if yes, wait for scanning new input
		do_lcd_command 0b10000000		; else, change control state to 'L'
		do_lcd_data 'L'
		cpi w3, 3						; if opaqe level == 3, wait for scanning new input, cannot increase anymroe
		breq return5
		inc w3							; else w3++, opaqe level increase
		do_lcd_command 0b11001010		; move to w3 condition number postiion
		do_lcd_data_num w3				; change w3 condition number
		jmp led_5A6						; jump to change w3 opaque level
; window 3 opaque level 
led_5A6:
		;rcall sleep_100ms
		ldi temp1, 85					; full duty cycle is 255, 85 = 255/3, 4 level
		mul w3, temp1					; duty cycle mul w3 condition number
		mov temp1, r0
		sts OCR3AL, temp1				; change the Timer PWM duty cycle
		CLR temp1						
		STS OCR3Ah, temp1				; clear high position of Timer PWM duty cycle
		jmp pop_v
...
; press nutton '6'
 num6:
		ldi temp1, 0					; justify whether in central control
		cp central, temp1
		brne return6					; if yes, wait for scanning new input
		do_lcd_command 0b10000000		; else, change control state to 'L'
		do_lcd_data 'L'
		cpi w3, 0						; if opaqe level == 0, wait for scanning new input, cannot decrease anymore
		breq return6
		dec w3							; else w3--, opaqe level decrease
		do_lcd_command 0b11001010		; move to w3 condition number postiion
		do_lcd_data_num w3				; change w3 condition number
		jmp led_5A6						; jump to change w3 opaque level
...
; press nutton '7'
 num7:
		ldi temp1, 0					; justify whether in central control
		cp central, temp1
		brne return7					; if yes, wait for scanning new input
		do_lcd_command 0b10000000		; else, change control state to 'L'
		do_lcd_data 'L'
		cpi w4, 3						; if opaqe level == 3, wait for scanning new input, cannot increase anymroe
		breq return7
		inc w4							; else w4++, opaqe level increase
		do_lcd_command 0b11001101		; move to w4 condition number postiion
		do_lcd_data_num w4				; change w4 condition number
		jmp led_7A8						; jump to change w4 opaque level
; window 4 opaque level 
led_7A8:
		;rcall sleep_100ms
		ldi temp1, 85					; full duty cycle is 255, 85 = 255/3, 4 level
		mul w4, temp1					; duty cycle mul w4 condition number
		mov temp1, r0
		sts OCR1AL, temp1				; change the Timer PWM duty cycle
		CLR temp1
		STS OCR1Ah, temp1				; clear high position of Timer PWM duty cycle
		jmp pop_v						
...
; press nutton '8'
 num8:
		ldi temp1, 0					; justify whether in central control
		cp central, temp1
		brne return8					; if yes, wait for scanning new input
		do_lcd_command 0b10000000		; else, change control state to 'L'
		do_lcd_data 'L'
		cpi w4, 0						; if opaqe level == 0, wait for scanning new input, cannot decrease anymore
		breq return8
		dec w4							; else w4--, opaqe level decrease
		do_lcd_command 0b11001101		; move to w4 condition number postiion	
		do_lcd_data_num w4				; change w1 condition number
		jmp led_7A8		
